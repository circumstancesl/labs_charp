...
namespace dichotomy_method
{
    public partial class NewtonForm : Form, IView
    {
        ...
        private bool ValidateText()
        {
            ...
            double limitMinus;
            double limitPlus;
            double numericalDifferentation;
            ...
            else if (string.IsNullOrEmpty(txtBoxSecondIntervalLim.Text) || (mathces = regex.IsMatch(txtBoxSecondIntervalLim.Text)) == false || !double.TryParse(txtBoxSecondIntervalLim.Text, out numericalDifferentation) || numericalDifferentation <= 0)
            {
                result = false;
                MessageBox.Show("Ошибка ввода значения численного дифференциала", "Ошибка ввода", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            else if (string.IsNullOrEmpty(txtBoxEpsilon.Text) || (mathces = regex.IsMatch(txtBoxEpsilon.Text)) == false || !double.TryParse(txtBoxEpsilon.Text, out epsilon) || epsilon <= 0)
            {
                result = false;
                MessageBox.Show("Ошибка ввода значения epsilon", "Ошибка ввода", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            else if (string.IsNullOrEmpty(txtBoxLimitation.Text) || (mathces = regex.IsMatch(txtBoxLimitation.Text)) == false || !double.TryParse(txtBoxLimitation.Text, out limitation) || limitation < 0)
            {
                result = false;
                MessageBox.Show("Ошибка ввода значения требуемой точности", "Ошибка ввода", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            else if (string.IsNullOrEmpty(txtBoxInterval.Text) || (mathces = regex.IsMatch(txtBoxInterval.Text)) == false || !double.TryParse(txtBoxInterval.Text, out interval) || interval <= 0)
            {
                result = false;
                MessageBox.Show("Ошибка ввода значения числа точек построения осей", "Ошибка ввода", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            else if (string.IsNullOrEmpty(txtBoxFunctionLimit.Text) || (mathces = regex.IsMatch(txtBoxFunctionLimit.Text)) == false || !double.TryParse(txtBoxFunctionLimit.Text, out limitMinus) || limitMinus <= 0)
            {
                result = false;
            ...
            else if (string.IsNullOrEmpty(txtBox.Text) || (mathces = regex.IsMatch(txtBox.Text)) == false || !double.TryParse(txtBox.Text, out limitPlus) || limitPlus <= 0)
            {
                result = false;
            ...
            else if (string.IsNullOrEmpty(txtBoxIteration.Text) || (mathces = regex.IsMatch(txtBoxIteration.Text)) == false || !double.TryParse(txtBoxIteration.Text, out iteration) || iteration <= 0)
            {
                result = false;
